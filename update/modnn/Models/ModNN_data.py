...
class external_CNN(nn.Module):
    """
    CNN-based External Module to learn heat gain from T_zone, T_ambient, and Solar radiation.
    Models thermal lag with a convolution kernel of 6 hours (24 time steps).
    """

    def __init__(self, input_channels=3, kernel_size=24):
        super(external_CNN, self).__init__()
        self.conv1 = nn.Conv1d(in_channels=input_channels, out_channels=8,
                               kernel_size=kernel_size, padding=0, bias=False)
        self.relu = nn.ReLU()
        self.conv2 = nn.Conv1d(in_channels=8, out_channels=1, kernel_size=1, padding=0, bias=False)
        self.fc = nn.Linear(1, 1)

    def forward(self, x_input, hidden_state=None):
        # Input: (batch, window, features) → select 3 features and permute
        x = x_input[:, :, [0, 1, 2]].permute(0, 2, 1)  # → (B, 3, T)
        out = self.relu(self.conv1(x))  # → (B, 8, T - k + 1)
        out = self.relu(self.conv2(out))  # → (B, 1, T - k + 1)
        out = out[:, :, -1:]  # → take last time step → (B, 1, 1)
        return self.fc(out.transpose(1, 2)), None  # → (B, 1, 1)
...
class ModNN(nn.Module):
    ...
    def forward(self, input_X):
        ...
        # zone_hidd = torch.ones(1, input_X.shape[0], self.Zone.dym.hidden_size).to(self.device)
        ...
        for i in range(window_size, self.encoLen):
            ...
            # deltaT, zone_hidd = self.Zone(total_flux, zone_hidd)
            deltaT = self.Zone(total_flux)
            E_Zone_T =  input_X[:, i:i+1, [0]] + deltaT
