...
class EV:
    """
    EV module for building-to-grid simulation.

    Attributes:
        capacity (float): Nominal capacity of the EV in kWh
    ...
    def __init__(self, capacity, max_charge_rate=0.25, max_discharge_rate=0.25,
        ...
        """
        Initialize the EV with given parameters.
    ...
    def simulate(self, power_series, occ_series):
        """
        Simulate EV operation.
        ...
        for t in range(power_series.shape[0]):
            ...
            if power > 0:  # Charging
                ...
                actual_power = min(power, self.max_charge_power*occ)
                # power to energy
                energy_to_EV = actual_power * self.resolution * self.efficiency_charge
                # feasible charing power constrained by max soc
                actual_energy_to_EV = min(energy_to_EV, (self.soc_max-self.soc) * self.capacity)
                # update soc
                self.soc += actual_energy_to_EV/self.capacity
                self.power_tracking.append(actual_energy_to_EV/self.resolution)
            elif power == 0:  # Idle
                ...
                # feasible charing power constrained by max_charge_power
                actual_power = max(power, -self.max_discharge_power*occ)
                # power to energy
                energy_from_EV = actual_power * self.resolution / self.efficiency_discharge
                # feasible charing power constrained by max soc
                actual_energy_from_EV = max(energy_from_EV, (self.soc_min - self.soc) * self.capacity)
                # update soc
                self.soc += actual_energy_from_EV / self.capacity
                self.power_tracking.append(actual_energy_from_EV/self.resolution)
